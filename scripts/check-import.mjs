...
const run=new (Object.getPrototypeOf(async function(){}).constructor)('pg','createHash','S3Client','GetObjectCommand','PutObjectCommand','HeadObjectCommand','DeleteObjectCommand','process','console',source);
...
async function scenario({done=false,fail=false,cross=false}={}){
 ...
 class S3{async send(cmd){if(fail)throw Object.assign(Error(),{code:'Denied'});if(cmd instanceof Get)return {Body:{transformToByteArray:async()=>new TextEncoder().encode('ok')}};if(cmd instanceof Head)return {ContentLength:2};return {};}}
 await run({Pool:class{async connect(){return db;}async end(){}}},createHash,S3,Get,Put,Head,Head,proc,{log(){},error(){}});
